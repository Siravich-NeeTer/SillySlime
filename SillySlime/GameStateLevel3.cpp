...
static irrklang::ISoundSource* gameEndSound;
...
void GameStateLevel3Init(void)
{
		...
		for (int x = 0; x < MAP_WIDTH; x++)
		{
			...
			case 12:
			{
				obj = gameObjInstCreate(GAMEOBJ_TYPE::TYPE_ENEMY_WARRIOR, glm::vec3(x + 0.5f, (MAP_HEIGHT - y) - 0.5f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f),
					glm::vec3(3.0f, 3.0f, 1.0f), 0.0f, true, 2, 0, 0.5f);
				weapon = dynamic_cast<Weapon*>(gameObjInstCreate(GAMEOBJ_TYPE::TYPE_WEAPON_SWORD, glm::vec3(obj->getPosition().x, obj->getPosition().y, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f),
					glm::vec3(3.0f, 3.0f, 1.0f), 0.0f, false, 2, 0, 0.25f));
				dynamic_cast<Enemy*>(obj)->setWeapon(weapon);
				dynamic_cast<Enemy*>(obj)->setMaxHP(200.0f);
				weapon->setHolder(obj);
				weapon->setCooldown(1.0f);
				weapon->setATK(80.0f);
				break;
			}
			case 13:
			{
				obj = gameObjInstCreate(GAMEOBJ_TYPE::TYPE_PINK_SLIME, glm::vec3(x + 0.5f, (MAP_HEIGHT - y) - 0.5f, 0.0f), glm::vec3(0.0f, 0.0f, 0.0f),
					glm::vec3(1.0f, 1.0f, 1.0f), 0.0f, false, 0, 0, 0.0f);
				obj->setCollision(true);
				break;
			}
	...
	BGM = SoundEngine->addSoundSourceFromFile("Stage3_BGM.wav");		//loop or not
	...
	gameEndSound = SoundEngine->addSoundSourceFromFile("GameEnd.wav");
	gameEndSound->setDefaultVolume(0.3f);
	...
	for (int i = 0; i < GAME_OBJ_INST_MAX; i++) {
		...
		for (int j = 0; j < GAME_OBJ_INST_MAX; j++)
		{
			...
			if (pInst1->getType() == TYPE_PLAYER)
			{
				...
				else if (pInst2->getType() == GAMEOBJ_TYPE::TYPE_PINK_SLIME && collide)
				{
					gameEnd = true;
					GameObject* obj = gameObjInstCreate(GAMEOBJ_TYPE::TYPE_UI_MISSION_COMPLETE, glm::vec3((sLimitRight + sLimitLeft) / 2.0f, 10.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f),
						glm::vec3(20.0f, 20.0f, 1.0f), 0.0f, false, 0, 0, 0.0f);
					SoundEngine->stopAllSounds();
					SoundEngine->play2D(gameEndSound, true);
				}
